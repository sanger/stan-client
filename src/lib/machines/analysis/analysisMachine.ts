import { ExtractResult, PassFail, RecordRnaAnalysisMutation, RnaAnalysisRequest } from '../../../types/sdk';
  ...
  },
  {
    actions: {
      ...
      removeExtractResult: assign((ctx, e) => {
        ...
        ctx.extractResults = ctx.extractResults.filter((res) => res.labware.barcode !== e.barcode);
      }),
      ...
      assignServerErrors: assign((ctx, e) => {
        if (!(e.type === 'error.platform.analysis' || e.type === 'error.platform.extractResult')) {
          return;
        }
