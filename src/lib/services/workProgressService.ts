import { FindWorkProgressQuery, FindWorkProgressQueryVariables, WorkStatus } from '../../types/sdk';
  ...
  implements SearchServiceInterface<FindWorkProgressQueryVariables, WorkProgressResultTableEntry>
{
   ...
   */
  formatFindResult = (findResult?: FindWorkProgressQuery['workProgress']): WorkProgressResultTableEntry[] => {
    if (!findResult) {
      return [];
    }
    ...
    return findResult.map((entry) => {
      ...
      entry.timestamps.forEach((timeStamp) => {
        timeStampMap.set(timeStamp.type as WorkProgressTimeStampType, timeStamp.timestamp);
      });
      ...
      return {
        ...
        lastRNAExtractionDate: lastRNAExtractionDate && new Date(lastRNAExtractionDate.toString()),
        lastCDNADate: lastCDNADate && new Date(lastCDNADate.toString()),
        lastStainTODate: lastStainVisiumTODate && new Date(lastStainVisiumTODate.toString()),
        lastStainLPDate: lastStainVisiumLPDate && new Date(lastStainVisiumLPDate.toString()),
        lastRNAScopeIHCStainDate: lastRNAScopeIHCStainDate && new Date(lastRNAScopeIHCStainDate.toString()),
        lastRNAAnalysisDate: lastRNAAnalysisDate && new Date(lastRNAAnalysisDate.toString()),
    ...
    .map((entry) => {
      return columns
        .map((column) => {
          ...
          throw new Error('createHistoryFileContent requires all column accessors to be strings');
        })
        .join(delimiter);
    })
    .join('\n');
