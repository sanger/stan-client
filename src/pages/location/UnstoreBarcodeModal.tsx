...
const UnstoreBarcodeModal: React.FC<UnstoreBarcodeModalProps> = ({ isOpen, location, barcode, onConfirm, onClose }) => {
  return (
    <Modal show={isOpen}>
      <ModalHeader>Unstore Barcode</ModalHeader>
      <ModalBody>
        <p className="text-sm text-gray-700">
          Are you sure you want to remove <span className="font-semibold">{barcode}</span> from{' '}
          <span className="font-semibold">{location.customName || location.fixedName || location.barcode}</span>?
        </p>
      </ModalBody>
      <ModalFooter>
