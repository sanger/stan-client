...
interface SectioningLayoutProps {
   ...
   */
  onCreate: (sectioningLayoutId: string, plan: PlanMutation["plan"]) => void;
  ...
  /**
   * If a plan has already been created and we want to show it, pass it in as `initialPlan`
   */
  initialPlan?: PlanMutation["plan"];

  /**
   * If a layout plan has already been created and we want to show it, pass it in as `initialLayoutPlan`
   */
    ...
    useEffect(() => {
      const subscription = service.subscribe((state) => {
        if (state.matches("done") || state.matches("printing")) {
          if (state.context.plan != null) {
            onCreate(state.context.sectioningLayout.cid, state.context.plan);
          }
        }
      });
              ...
              <LayoutPlanner actor={layoutMachine}>
                <div className="my-2">
                  <p className="text-gray-900 text-sm leading-normal">
                    ...
                    on the right, and then click a destination slot. Multiple
                    sections may be added to a slot by clicking it multiple
                    times.
                  </p>
