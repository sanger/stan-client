...
describe('CytAssist Page', () => {
  ...
  context('when scanning a labware missing Probe Hybridisation CytAssist or QC', () => {
    ...
    it('should display a warning message', () => {
      cy.contains(
        ...
        "Labware STAN-1410 is missing the following operations (not required for 3'): 'Probe hybridisation Cytassist' , 'Probe hybridisation QC'"
      );
    });
    after(() => {
  ...
  context('scanned input labware are displayed within the same view', () => {
    before(() => {
      cy.get('#labwareScanInput').type('STAN-1410{enter}');
      cy.get('#labwareScanInput').type('STAN-1420{enter}');
    });

    it('shows it on the page', () => {
      cy.findByText('STAN-1410').should('be.visible');
      cy.findByText('STAN-1420').should('be.visible');
    });
  ...
  context('Mapping', () => {
    ...
    it('displays the table with A1, B1 slots mapped to D1', () => {
      cy.findByTestId('mapping_table').get('tbody tr').should('have.length', 4);
    ...
    context('multi select from different source labware is forbidden', () => {
      before(() => {
        ...
        cy.findByTestId('STAN-1420').within(() => {
...
const fillForm = () => {
  ...
  cy.get('#labwareScanInput').type('STAN-1410{enter}').wait(500);
