...
describe('History Page', () => {
  ...
  describe('Result Format is set to Graph', () => {
    context('when I visit the page with good URL params', () => {
      before(() => cy.visit('/history?barcode=STAN-1001&resultFormat=graph'));

      it('performs a history graph search', () => {
        cy.findByText('History Graph').should('be.visible');
      });
    });
    context('when updating the font size', () => {
      before(() => cy.visit('/history?barcode=STAN-1001&resultFormat=graph&fontSize=20'));

      it('performs a history graph search', () => {
        shouldDisplaySelectedValue('font-size-select', '20');
      });
    });
    context('when zooming in to the maximum (10)', () => {
      before(() => cy.visit('/history?barcode=STAN-1001&resultFormat=graph&fontSize=20&zoom=10'));

      it('disables the zoom in button', () => {
        cy.findByTestId('zoom-in-icon').should('have.class', 'cursor-not-allowed');
      });
    });
    context('when zooming out to the minimum (0.1)', () => {
      before(() => cy.visit('/history?barcode=STAN-1001&resultFormat=graph&fontSize=20&zoom=0.1'));

      it('disables the zoom out button', () => {
        cy.findByTestId('zoom-out-icon').should('have.class', 'cursor-not-allowed');
      });
    });
  });
});
