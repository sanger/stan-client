...
describe('History Page', () => {
  ...
  describe('when Plot search is performed', () => {
    const props: HistoryUrlParams = { workNumber: 'SGP1008', resultFormat: 'graph' };
    beforeEach(() => {
      jest.spyOn(xState, 'useMachine').mockReturnValue([
        {
          value: 'found',
    ...
    it('displays the result as  graph', () => {
      expect(screen.getByText('History Graph')).toBeVisible();
    });
    it('displays zoom in/out buttons', () => {
      expect(screen.getByTestId('zoom-in-icon')).toBeVisible();
      expect(screen.getByTestId('zoom-out-icon')).toBeVisible();
    });
    it('displays font size select box', () => {
      expect(screen.getByTestId('font-size-select')).toBeVisible();
    });
    it('default the font size select box to 16', () => {
      expect(screen.getByTestId('font-size-select')).toHaveTextContent('16');
    });
    it('displays download graph button', () => {
      expect(screen.getByTestId('download-button')).toBeInTheDocument();
    });
    it('hides flagged labware section', () => {
      expect(screen.queryByText('Flagged Labware')).not.toBeInTheDocument();
    });
    it('hides uploaded files section', () => {
